'use strict';
const CACHE_VERSION='wellone-customer-v82-exact-variant-sync';
...
const SHELL=[
  ...
  './contact.html',
  './css/style.css?v=82',
  './js/store.bundle.js?v=82',
  './js/basic.bundle.js?v=82',
  './js/orders.bundle.js?v=82',
...
self.addEventListener('activate',event=>{
  event.waitUntil(Promise.all([
    caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('wellone-customer-')&&!key.startsWith(CACHE_VERSION)).map(key=>caches.delete(key)))),
    self.clients.claim()
  ]));
});
function isSupabase(request){try{return new URL(request.url).hostname.endsWith('.supabase.co');}catch(_e){return false;}}
async function trim(name,max=220){const cache=await caches.open(name);const keys=await cache.keys();if(keys.length>max)await Promise.all(keys.slice(0,keys.length-max).map(k=>cache.delete(k)));}
